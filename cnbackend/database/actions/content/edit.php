...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $id = $_POST["id"];
    $d = $db->CustomQuery("SELECT slug FROM contents WHERE id={$id}");
    $sl = $d[0]->slug;
    $s = $db->CustomQuery("SELECT * FROM {$slugs_table} WHERE slug='$sl'");
    if (empty($s)) {
        $params = ["page_name " => "contents", "slug" => $sl];
    ...
    if ($db->Update("contents", $insert_params, "id", [$id])) {
        $params = ["slug" => $slug];
