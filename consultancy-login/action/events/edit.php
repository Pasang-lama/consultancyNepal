 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
     ...
     if(!empty($errors)){
        ...
        $_SESSION["old"]=$old;
            ...
            echo 'window.location.href="https://www.consultancynepal.com/consultancy-login/events/editevents.php?id=' . $id . '"';
            echo "</script>";
         die();
    }else{
     ...
     $d = $db->CustomQuery("SELECT slug FROM events WHERE id={$id}");
     $sl = $d[0]->slug;
     $gets = $db->CustomQuery("SELECT * FROM slugs WHERE slug='{$sl}'");
     if (empty($gets)) {
         $params = ["page_name" => "events", "slug" => $sl];
     ...
     if ($db->Update("events", $insert_params, "id", [$id])) {
         $params = ["page_name" => "events", "slug" => $slug];
         $db->Update("slugs", $params, "slug", [$sl]);
         
         $_SESSION["message"] = "EVENTS Edited Successfully";
         echo '<script type="text/javascript">';
         echo 'window.location.href="https://www.consultancynepal.com/consultancy-login/events/showevents";';
         echo "</script>";
         die();
     } else {
         $_SESSION["messages"] = "EVENTS Edited failed";
         echo '<script type="text/javascript">';
         echo 'window.location.href="https://www.consultancynepal.com/consultancy-login/events/showevents";';
         echo "</script>";
         die();
         header(
             "location:https://www.consultancynepal.com/consultancy-login/events/showevents"
         );
     }
 }
