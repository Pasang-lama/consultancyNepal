 <?php class Database
 {
     ...
     private function Connection()
     {
         try {
             $this->_connection = new PDO(
                 "mysql:host=localhost;dbname=consulta_consultancynepal",
                 "consulta_consultancynepal",
                 "admin@002"
             );
     ...
     public function Insert($tableName, $data = [])
     {
         if (empty($tableName) || empty($data)) {
             throw new PDOException("Table name & data field is mandatory");
         }
     ...
     public function Update(
         ...
         if (
             ...
             throw new PDOException("Criteria not match");
         }
     ...
     {
         if (empty($tableName) || empty($criteria) || empty($bindValue)) {
             throw new PDOException("Not valid criteria");
         }
     ...
     public function SelectAll($tableName = "", $bindData = [])
     {
         if (empty($tableName)) {
             throw new PDOException("Table name is required");
         }
     ...
     {
         if (empty($query)) {
             throw new PDOException("Query field is required");
         }
         $prepareStatement = $this->_connection->prepare($query);
         try {
