...
$datas = $db->CustomQuery("select * from `best-consultancy-locationwise` where slug='$url[1]'");
...
//   print_r($parent_table);

// $child_data = $db->CustomQuery("SELECT consultancies.*,consultancyTestPreparationDeatils.details,image FROM consultancy_test_prep JOIN consultancies on consultancy_test_prep.cid = consultancies.id JOIN consultancyTestPreparationDeatils on consultancy_test_prep.id=consultancyTestPreparationDeatils.ctpid   WHERE consultancy_test_prep.tpid =$data->extras  ORDER BY consultancy_test_prep.rank ASC");
$child_data = $db->CustomQuery("SELECT consultancies.* FROM rank_consultancy JOIN consultancies on rank_consultancy.consultancy_id= consultancies.id    WHERE consultancies.City =$data->extras  ORDER BY rank_consultancy.rank ASC");
// print_r($child_data);
// die;
?>
