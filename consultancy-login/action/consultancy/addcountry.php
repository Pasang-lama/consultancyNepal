<?php 
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($auth_result as $datas) {
        $previous_aid[] = $datas->country_id;
    }
    if ($present_aid == null) {
        if ($previous_aid != null) {
            foreach ($previous_aid as $data) {
                $db->CustomQuery(
                    "DELETE FROM consultancy_pages WHERE country_id={$data} AND consultancy_id={$cid}"
                );
            }
        }
        ...
        foreach ($present_aid as $data) {
            $db->Insert($cp, ["consultancy_id" => $cid, "country_id" => $data]);
        }
        $_SESSION["message"] = "Changes Made Successfully";
        echo '<script type="text/javascript">';
        echo 'window.location.href=" https://www.consultancynepal.com/consultancy-login/consultancy/showconsultancy";';
        ...
        if (count($present_aid) > count($previous_aid)) {
            ...
            $_SESSION["message"] = "coountry Added to the Consultancy ";
            echo '<script type="text/javascript">';
            echo 'window.location.href=" https://www.consultancynepal.com/consultancy-login/consultancy/showconsultancy";';
            echo "</script>";
