<?php 
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
     $cp = "consultancy_test_prep";
     session_start();
     $present_aid = $_POST["consultancy_list"];
     $cid = $_POST["country"];
     require_once "../../database/database.php";
     require_once "../../helper/tables.php";
     require_once "../../helper/config.php";
     $db = Database::Instance();
     $previous_aid = [];
     $auth_result = $db->SelectByCriteria(
         "consultancy_test_prep",
         "tpid",
         "cid",
         [$cid]
     );
     foreach ($auth_result as $datas) {
         $previous_aid[] = $datas->tpid;
     }
    //  echo "<pre>";
    //  print_r($present_aid);
    //  print_r($previous_aid);
     if ($present_aid == null) {
         if ($previous_aid != null) {
             foreach ($previous_aid as $data) {
                 $db->CustomQuery(
                     "DELETE FROM consultancy_test_prep WHERE cid={$cid} AND tpid={$data}"
                 );
             }
         }
         $_SESSION["message"] = "Changes Made Successfully";
         echo '<script type="text/javascript">';
         echo 'window.location.href=" https://www.consultancynepal.com/consultancy-login/consultancy/showconsultancy";';
         echo "</script>";
     } elseif ($previous_aid == null && $present_aid != null) {
         foreach ($present_aid as $data) {
             $db->Insert($cp, ["tpid" => $data, "cid" => $cid]);
         }
         $_SESSION["message"] = "Changes Made Successfully";
         echo '<script type="text/javascript">';
         echo 'window.location.href=" https://www.consultancynepal.com/consultancy-login/consultancy/showconsultancy"';
         echo "</script>";
     } else {
         if (count($present_aid) > count($previous_aid)) {
             $inserting_aid = array_diff($present_aid, $previous_aid);
             foreach ($inserting_aid as $aids) {
                 $aut_book_params = ["tpid" => $aids, "cid" => $cid];
                 $db->Insert($cp, $aut_book_params);
             }
             $result = $db->SelectByCriteria(
                 "consultancy_test_prep",
                 "tpid",
                 "cid",
                 [$cid]
             );
             $table_authors = [];
             foreach ($result as $data) {
                 $table_authors[] = $data->tpid;
             }
             $re_data = array_diff($table_authors, $present_aid);
             if (count($re_data) != 0) {
                 foreach ($re_data as $data) {
                     $db->CustomQuery(
                         "DELETE FROM consultancy_test_prep WHERE cid={$cid} AND tpid={$data}"
                     );
                 }
             }
             $_SESSION["message"] = "Consultancy Added to the coountry";
             echo '<script type="text/javascript">';
             echo 'window.location.href=" https://www.consultancynepal.com/consultancy-login/consultancy/showconsultancy"';
             echo "</script>";
         } elseif (count($previous_aid) > count($present_aid)) {
             $new_a = array_diff($present_aid, $previous_aid);
             if (count($new_a) != 0) {
                 foreach ($new_a as $aids) {
                     $aut_book_params = ["tpid" => $aids, "cid" => $cid];
                     $db->Insert($cp, $aut_book_params);
                 }
                 $result = $db->SelectByCriteria($cp, "tpid", "cid", [$cid]);
                 $table_authors = [];
                 foreach ($result as $data) {
                     $table_authors[] = $data->tpid;
                 }
                 $re_data = array_diff($table_authors, $present_aid);
                 if (count($re_data) != 0) {
                     foreach ($re_data as $data) {
                         $db->CustomQuery(
                             "DELETE FROM consultancy_test_prep WHERE cid={$cid} AND tpid={$data}"
                         );
                     }
                 }
             } else {
                 $new_b = array_diff($previous_aid, $present_aid);
                 if (count($new_b) != 0) {
                     foreach ($new_b as $data) {
                         $db->CustomQuery(
                             "DELETE FROM consultancy_test_prep WHERE cid={$cid} AND tpid={$data}"
                         );
                     }
                 }
             }
             $_SESSION["message"] = "Consultancy Added to the coountry";
             echo '<script type="text/javascript">';
             echo 'window.location.href=" https://www.consultancynepal.com/consultancy-login/consultancy/showconsultancy"';
             echo "</script>";
         } elseif (count($present_aid) == count($previous_aid)) {
             $diff = array_diff($present_aid, $previous_aid);
             if (count($diff) != null) {
                 foreach ($diff as $aids) {
                     $aut_book_params = ["tpid" => $aids, "cid" => $cid];
                     $db->Insert($cp, $aut_book_params);
                 }
                 $result = $db->SelectByCriteria($cp, "tpid", "cid", [$cid]);
                 $table_authors = [];
                 foreach ($result as $data) {
                     $table_authors[] = $data->tpid;
                 }
                 $re_data = array_diff($table_authors, $present_aid);
                 if (count($re_data) != 0) {
                     foreach ($re_data as $data) {
                         $db->CustomQuery(
                             "DELETE FROM consultancy_test_prep WHERE cid={$cid} AND tpid={$data}"
                         );
                     }
                 }
                 $_SESSION["message"] = "Consultancy Added to the coountry";
                 echo '<script type="text/javascript">';
                 echo 'window.location.href="  https://www.consultancynepal.com/consultancy-login/consultancy/showconsultancy"';
                 echo "</script>";
             }
         }
     }
 }